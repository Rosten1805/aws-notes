...
## Ejemplo de organización real

```text
Grupo: Operaciones      Grupo: Ventas      Grupo: Administradores
- David                 - Raúl             - Joan
- Luis                             - Gabriel
                                    - George

David  → pertenece a Operaciones y también a Administradores (dos grupos)
Miguel → no pertenece a ningún grupo (permisos individuales)
```

Es habitual que un mismo usuario pertenezca a varios grupos (ej. David necesita permisos de Operaciones y también de Administración), y también es normal dejar a alguien fuera de cualquier grupo cuando su acceso es un caso muy específico que no encaja con el resto del equipo (ej. Miguel).
...
Repositorio oficial de la AWS CLI: https://github.com/aws/aws-cli
...
## Ejemplo: acceso entre cuentas (cross-account)

Un caso muy típico en empresas con varias cuentas AWS (ej. `Producción` y `Auditoría`):

```text
Usuario en cuenta Auditoría
        │  sts:AssumeRole
        ▼
Rol "AuditorReadOnly" en cuenta Producción
        │
        ▼
Credenciales temporales de solo lectura sobre Producción
```

En vez de crear un usuario IAM duplicado en la cuenta de Producción, el equipo de Auditoría **asume un rol** que ya existe allí. La política de confianza del rol especifica que solo la cuenta de Auditoría puede asumirlo, y la política de permisos limita el acceso a solo lectura. Así nunca hay credenciales permanentes compartidas entre cuentas.
