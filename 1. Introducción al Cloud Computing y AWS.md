...
Normalmente una región tiene 3 AZ (mínimo 3, máximo 6), nombradas como `eu-west-2a`, `eu-west-2b`, `eu-west-2c` (región de Londres).

---

## Ejemplo progresivo: de 1 AZ a multi-región

Imagina que despliegas una aplicación en Londres (`eu-west-2`) y vas resolviendo problemas a medida que aparecen:

```mermaid
flowchart TB
    subgraph Paso1["1. Una sola AZ"]
        A1[App en AZ única]
    end
    subgraph Paso2["2. Varias AZ en Londres"]
        A2[App en AZ 1]
        A3[App en AZ 2]
    end
    subgraph Paso3["3. Multi-región"]
        A4[Londres: App en AZ 1 y AZ 2]
        A5[N. Virginia: App en AZ 1 y AZ 2]
    end
    Paso1 -->|"Si falla la AZ, la app cae entera"| Paso2
    Paso2 -->|"Si falla Londres entero, la app sigue caída"| Paso3
```

| Problema | Con 1 AZ | Con varias AZ (misma región) | Con multi-región |
|---|---|---|---|
| Usuarios lejanos tienen latencia alta | ❌ Sin resolver | ❌ Sin resolver | ✅ Resuelto (sirves desde la región más cercana) |
| Falla una AZ | ❌ La app cae | ✅ Sigue disponible desde otra AZ | ✅ Sigue disponible |
| Falla la región entera (ej. Londres) | ❌ La app cae | ❌ La app cae | ✅ Sigue disponible desde N. Virginia |

Este es el razonamiento que justifica por qué las arquitecturas críticas en AWS casi siempre se diseñan **multi-AZ** como mínimo, y **multi-región** cuando la disponibilidad global es un requisito de negocio.
...
**Ejemplo**: la región N. Virginia (`us-east-1`) tiene Local Zones en Boston, Chicago, Dallas, Miami, Houston, entre otras. Una empresa con clientes en Miami que necesita muy baja latencia puede desplegar recursos en la Local Zone de Miami sin tener que esperar a que AWS abra una región completa allí.
