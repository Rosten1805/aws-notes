...
# Truco práctico: crear varias cuentas con un solo Gmail

Cada cuenta de AWS necesita un email distinto para registrarse, pero si usas Gmail no hace falta crear una bandeja de entrada nueva por cada cuenta: Gmail ignora todo lo que pongas entre un `+` y la `@` de tu dirección, así que los emails con `+algo` siguen llegando a tu bandeja principal.

Ejemplo, partiendo de `blockstellart@gmail.com`:

```text
blockstellart@gmail.com                  → cuenta principal / Root de gestión
blockstellart+AWSAccount1@gmail.com      → cuenta Production
blockstellart+AWSAccount2@gmail.com      → cuenta Development
blockstellart+AWSAccount3@gmail.com      → cuenta Sandbox
```

No es necesario configurar estos alias de antemano en Gmail: funcionan directamente, y todos los correos de verificación de AWS llegarán a la misma bandeja de `blockstellart@gmail.com`.
